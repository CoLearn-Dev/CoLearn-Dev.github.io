...
pub async fn init_and_run(address: String, port: u16) -> Result<(), Box<dyn std::error::Error>> {
    set_jwt();
    tokio::spawn(print_admin_token());

    let socket_address = format!("{}:{}", address, port).parse()?;
    run_server(socket_address).await?;
    Ok(())
}

pub async fn run_server(socket_address: SocketAddr) -> Result<(), Box<dyn std::error::Error>> {
